...
export const handler = async (event) => {
  ...
  await store.setJSON(order.id, { ...order, status: 'processing', telegramMsgId: null });
  ...
  if (result.ok) {
    await store.setJSON(order.id, { ...order, status: 'processing', telegramMsgId: result.result.message_id });
  }
