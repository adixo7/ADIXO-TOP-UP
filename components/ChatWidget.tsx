    ...
    <div className="fixed bottom-24 md:bottom-6 right-6 z-[200] flex flex-col items-end">
      {isOpen && (
        <div className="w-80 md:w-96 h-[500px] bg-zinc-950 border border-zinc-800 rounded-3xl shadow-[0_0_50px_-12px_rgba(0,0,0,0.8)] flex flex-col mb-4 overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
          {/* Header */}
          <div className={`p-4 flex items-center justify-between transition-colors duration-500 ${modelType === 'pro' ? 'bg-orange-900/40 border-b border-orange-500/30' : 'bg-zinc-900 border-b border-zinc-800'}`}>
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all ${modelType === 'pro' ? 'bg-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.5)]' : 'bg-zinc-800'}`}>
            ...
            <div className="flex bg-black/40 p-1 rounded-xl border border-zinc-800">
              ...
              <button 
                ...
                className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${modelType === 'pro' ? 'bg-orange-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
              >
                Elite
              </button>
            </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-zinc-950">
            ...
            {isLoading && (
              <div className="flex flex-col items-start">
                <div className="bg-orange-500/5 px-4 py-3 rounded-2xl rounded-tl-none border border-orange-500/10">
                ...
                <span className="text-[8px] font-black text-orange-500/50 uppercase tracking-widest mt-1 ml-1 animate-pulse">
          ...
          <div className="p-4 bg-zinc-900/50 border-t border-zinc-800">
            <div className="flex gap-2 bg-black/40 p-1.5 rounded-2xl border border-zinc-800 focus-within:border-orange-500/50 transition-all">
              <input 
                type="text" 
                value={input}
